...
namespace database
{
    Conf Conf::fromJSON(const std::string &str)
    {
        ...
        Poco::JSON::Object::Ptr object_array = object->getObject("report");
        conf._report.report_text = object_array->getValue<std::string>("report_text");
        conf._report.author_id = object_array->getValue<long>("author_id");
        conf._report.date_create_report = object_array->getValue<std::string>("date_create_report");
    ...
    const std::string  Conf::get_report_as_string() const
    {
        return _report.to_string();
    }

    const std::string Conf::Report::to_string() const
    {
        std::stringstream ss;
        ss << report_text << ", " << author_id << ", " << date_create_report;
        return ss.str();
    }
    ...
    std::optional<Conf> Conf::read_by_id(long id)
    {
        ...
        params["id"] = id;
        std::vector<std::string> results = database::Database::get().get_from_mongo("confs",params);
    ...
    Poco::JSON::Object::Ptr Conf::toJSON() const
    {
        ...
        Poco::JSON::Object::Ptr report = new Poco::JSON::Object();

        report->set("report_text",_report.report_text);
        report->set("author_id",_report.author_id);
        report->set("date_create_report",_report.date_create_report);
